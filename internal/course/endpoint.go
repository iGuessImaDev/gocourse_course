...
func makeCreateEndpoint(s Service) Controller {
	return func(ctx context.Context, request interface{}) (interface{}, error) {
		...
		if req.Name == "" {
			return nil, response.BadRequestError(ErrNameRequired.Error())
		}

		if req.StartDate == "" {
			return nil, response.BadRequestError(ErrStartDateRequired.Error())
		}

		if req.EndDate == "" {
			return nil, response.BadRequestError(ErrEndDateRequired.Error())
		}

		course, err := s.Create(ctx, req.Name, req.StartDate, req.EndDate)
		if err != nil {
			if err == ErrEndBeforeStart || err == ErrInvalidStartDate ||
